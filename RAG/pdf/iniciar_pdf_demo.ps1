<#
    RAG sobre una carpeta de PDFs para clase.
    Uso:
      .\iniciar_pdf_demo.ps1                              -> modo interactivo
      .\iniciar_pdf_demo.ps1 "de que trata el doc?"       -> una sola pregunta
      .\iniciar_pdf_demo.ps1 --reindex "..."              -> reconstruye el indice
      .\iniciar_pdf_demo.ps1 --docs C:\ruta\a\pdfs "..."  -> usa otra carpeta de PDFs
#>
...
# 4. Correr la demo
Write-Paso "Iniciando RAG sobre PDFs..."
if ($Pregunta -and $Pregunta.Count -gt 0) {
    python rag_pdf.py @Pregunta
} else {
    python rag_pdf.py
}

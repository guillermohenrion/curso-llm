<#
    Graph RAG (RAG sobre un grafo de conocimiento) para clase.
    Uso:
      .\iniciar_graph_demo.ps1                  -> modo interactivo
      .\iniciar_graph_demo.ps1 "que es RAG?"    -> una sola pregunta y listo
#>
...
# 3. Activar el entorno virtual (esta 2 niveles arriba: RAG/graph/ -> RAG/ -> raiz)
...
# 4. Correr la demo
Write-Paso "Iniciando Graph RAG..."
if ($Pregunta -and $Pregunta.Count -gt 0) {
    python rag_grafos.py @Pregunta
} else {
